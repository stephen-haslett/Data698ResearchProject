...
```{r calculateNumberOfRandomGamesRequiredToWinJackpot, echo = FALSE, message = FALSE, warning = FALSE}
calculateRandomTicketsNeededToWin <- function() {
  games <- 0
  ticketCost <- 0
  jackpot = FALSE
  results <- list()
  while (isFALSE(jackpot)) {
    for (row in rownames(testData)) {
      games <- games + 1
      ticketCost <- ticketCost + 3
      # Generate the random lottery numbers to play against the test data set.
      lotteryNumbers <- data.frame()
      lotteryNumbers <- colnames('winning_numbers')
      random <- generateRandomLotteryTickets()
      lotteryNumbers$winning_numbers <- formatC(x = random[[1]], digits = 1, flag = '0', format = 'd')
      ...
      # Count how many matching winning numbers we get per game. 
      lotteryGame <- sapply(lotteryNumbers$winning_numbers, function(s) length(grep(s, testData[row, 'winning_numbers'])))
      totalMatches <- sum(lotteryGame)

      # Check if we get a Mega Ball match.
      matchingMegaBall <- megaBallMatch(lotteryNumbers$mega_ball, testData[row, 'mega_ball'])

      # When the amount of matching numbers required to win the jackpot is met, exit the loop by seting
      # the "jackpot" variable to TRUE.
      if (totalMatches == 3 && isTRUE(matchingMegaBall)) {
        jackpot = TRUE
      }
    }
  }
  ...
  if (isTRUE(jackpot)) {
    # Populate the results list so we can return the results.
    ticketCost <- currency(ticketCost)
    results <- append(games, ticketCost)
  }
  
  return(results)
}

gameResults <- calculateRandomTicketsNeededToWin()
games <- gameResults[[1]]
cost <- currency(gameResults[[2]])
games
cost
```
