...
#'
#' @return Dataframe: A dataframe of the most frequently occuring winning numbers.
#'
#' @export
generateFrequencyBasedLotteryTicket <- function(trainingData) {
  ...
  winningNumbers <- t(winningNumbers)
  winningNumbers <- as.data.frame(winningNumbers)
  
  # Combine winning numbers into one row. 
  winningNumbers <- mutate(winningNumbers, winning_numbers = paste(V1, V2, V3, V4, V5), .keep = 'none')
  ...
  lotteryNumbers <- cbind(winningNumbers, megaBallNumbers)
  lotteryNumbers <- as.data.frame(lotteryNumbers)
  setnames(lotteryNumbers, 'megaBallNumbers', 'mega_ball')
