...
#'
#' @return String: Total winning dollar amount based on numbers matched.
#'
#' @export
...
generateRandomLotteryTickets <- function(numberOfTickets, includeMultiplier = FALSE) {
  ...
  # Convert winning_numbers from character to numeric, and split them into the expected numeric 2 digit number sequence.
  # Winning numbers must be numeric so we can count how often they occur in the winning numbers test dataset.
  numericWinningNumbers <- as.numeric(regmatches(lotteryTickets$winning_numbers, gregexpr(".{2}", lotteryTickets$winning_numbers))[[1]])
  winning_numbers <- append(winning_numbers, numericWinningNumbers)

  # Remove the existing winning_numbers column form the dataframe so we can replace it with the reformatted version.
  lotteryTickets <- select(lotteryTickets, -winning_numbers)

  # TODO FIX THIS - GIVING "replacement has 5 rows, data has 1" ERROR.
  #lotteryTickets$winning_numbers <- NA
  lotteryTickets$winning_numbers <- winning_numbers

  return(lotteryTickets)
}
generateRandomLotteryTickets(1)
```
...
#'
#' @return Dataframe: Dataframe consisting of the 5 most frequent winning numbers, and the most
...
lotteryGameSimulation <- function(ticketNumbers = NULL, trainingData = NULL, testData, generationMethod) {
  ...
  # Count how many rows are in the testData set so we can generate an equal number
  # of random lottery tickets when generationMethod is set to "random".
  ...
  if (generationMethod == 'frequency' && !is.null(trainingData)) {
    # TODO: FIX THIS FOR FREQUENCY AND ALSO RANDOM - NOT WORKING.
    ...
    lotteryNumbers$winning_numbers <- formatC(x = frequencyNumbers$winning_numbers, digits = 1, flag = '0', format = 'd')
  }
...
class(const)
freq <- generateFrequencyBasedLotteryTicket(trainingData)

# TODO: FIGURE OUT WHY RANDOM AND FREQUENCY DO NOT GENERATE WINNING NUMBER MATCHES
