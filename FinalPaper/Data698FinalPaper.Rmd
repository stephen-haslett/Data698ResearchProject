...
In order to simulate both of these approaches, we first needed to find the numbers that conformed to each philosophy. To do this, we ran through the historical Mega Millions winning numbers dataset and calculated the frequency of both the most and least frequently occurring winning Mega Ball and Base numbers. We then ranked each number in accordance to it's frequency.
...
With the simulated lottery tickets in hand, the next step was to compare the winnings obtained from playing these tickets against those of our base case (_Consistent Numbers Strategy_) ...... in order to ascertain 
