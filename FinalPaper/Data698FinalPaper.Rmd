...
For ease of interpretation, the findings of this study have been segmented into sections corresponding to the strategies that they derive from. Each section consists of the T-Test results that were generated as a consequence of comparing the average winnings of the strategy under discussion, with those of the base case. Further, each section contains a visual representation of comparison results in the form of distribution histograms.
...
_Ho: mean(base) - mean(high_freq) = 0_
_Ha: mean(base) - mean(high_freq) != 0_
...
_Ho: mean(base) - mean(low_freq) = 0_
_Ha: mean(base) - mean(low_freq) != 0_
...
_Ho: mean(base) - mean(random) = 0_
_Ha: mean(base) - mean(random) != 0_
...
Based on our experiments, we can conclude that consistently picking the same numbers (_base case_) for every lottery game is just as effective/ineffective as randomizing your number choices every game, or picking numbers based on past results. However, we also found that paying an extra dollar for the multiplier number is strictly worse and typically results in a higher loss. In conclusion, we believe that the lottery is not worth playing as the average amount won is less than the amount spent on tickets. This average does not get better based on how numbers are picked, but does get worse when you pay more for a chance at a multiplier.
