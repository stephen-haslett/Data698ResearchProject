...
For our base case strategy (_The Consistent Numbers Strategy_) 
BASE CASE: We randomly generate 1 lottery, ticket (consstent strategy), and play the same ticket aganst a randomly generated winning ticket each time.

RANDOM NUMBER STRATEGY - We generate a different lottery ticket and play it aganst a randomly generated winning ticket - each game.

MULTIPLIER CASE - SAme as base case but turn multpliers on.

For the next 2 cases we leveraged the original dataset in order to select the most frequent and least frequent numbers. Looked at the historical dataset of winning tickets and rank ordered the occurence of indivudal numbers, and megaballs. We used to top 50 percentile to represent hot (most frequent), and the bottom 50 percentile to represent least frequent.

LEAST FREQUENT CASE
We randomly generate 1 lottery ticket based on only the least frequently occuring numbers and played the same ticket aganst a randomly generated winning ticket each time.
MOST FREQUENT CASE
We randomly generate 1 lottery ticket based on only the most frequently occuring numbers and played the same ticket aganst a randomly generated winning ticket each time.

We plan to compare each strategie to the base case based on the average winnings for each strategy.
